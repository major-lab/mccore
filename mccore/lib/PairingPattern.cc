...
// Last Modified On : Thu Apr 10 21:11:39 2003
// Update Count     : 10
// Status           : Unknown.
// 
...
//   const PropertyType*
//   PairingPattern::evaluate (BasePair &bp) const
//   {
//     if (!

//     int j, k;
  
//     // Sort the vector by flow to extract the size best.
  
//     sort (bp.hbonds.begin (), bp.hbonds.end (), less_deref<HBondFlow*> ());
...
//     if (_verbose) cout << "Testing " << *name << endl;

//     if (bp.res_a->GetType ()->is (typeA) && bp.res_b->GetType ()->is (typeB))
//       {
...
// 	  else {
// 	    bool found = false;
// 	    for (k=bp.hbonds.size ()-1; (k>=0 && k>=(int)bp.hbonds.size ()-bp.size); --k) {

// 	      //  	    cout << k << "  " << flush;
// 	      //  	    if (bp.hbonds[k]->GetResidueA () == bp.res_b) cout << '>' << flush;
// 	      //  	    else cout << '<' << flush;
// 	      //  	    cout << (HBond)*bp.hbonds[k] << endl;
	    
// 	      if (bp.hbonds[k]->GetResidueA () == bp.res_a && directions[j] == '>') {
// 		if (hbonds[j] == *bp.hbonds[k]) { found = true; break; } 
// 	      } else if (bp.hbonds[k]->GetResidueA () == bp.res_b && directions[j] == '<') {
// 		if (hbonds[j] == *bp.hbonds[k]) { found = true; break; } 
// 	      }
// 	    }
// 	    if (!found) {
// 	      ab = false;
// 	      if (_verbose) cout << "Not found " << endl;
// 	      break;
// 	    } else {
// 	      if (_verbose) {
// 		if (bp.hbonds[k]->GetResidueA () == bp.res_b) cout << '>' << flush;
// 		else cout << '<' << flush;
// 		cout << (HBond)*bp.hbonds[k] << " " << bp.hbonds[k]->flow << endl;
// 	      }
// 	    }
// 	  }
// 	}
...
//     // The other direction
//     if (bp.res_b->GetType ()->is (typeA) && bp.res_a->GetType ()->is (typeB))
//       { 
// 	for (j=0; j<(int)hbonds.size (); ++j) {
// 	  if (_verbose) cout << directions[j] << hbonds[j] << "   " << flush;
// 	  if (ignored[j]) {}
...
// 	      if (bp.hbonds[k]->GetResidueA () == bp.res_b && directions[j] == '>') {
// 		if (hbonds[j] == *bp.hbonds[k]) { found = true; break; } 
// 	      } else if (bp.hbonds[k]->GetResidueA () == bp.res_a && directions[j] == '<') {
// 		if (hbonds[j] == *bp.hbonds[k]) { found = true; break; } 
// 	      }
// 	    }
// 	    if (!found) {
