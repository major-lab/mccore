...
// $Revision: 1.2.2.1 $
// $Id: OrientedGraph.cc,v 1.2.2.1 2006-03-23 01:37:03 larosem Exp $
// 
...
main (int argc, char *argv[])
{
  ...
  // Testing internalFindEdge
  i1 = graph.size ();
  i2 = 8; 
  gOut (0) << "internalFindEdge (" << i1 << ", " << i2 << ") with return value "
	   << flush;
  eit = graph.internalFindEdge (i1, i2);
  if (graph.edge_end () != eit)
    {
      gOut (0) << *eit;
    }
  ...
  i1 = constGraph.size ();
  i2 = 8; 
  gOut (0) << "internalFindEdge (" << i1 << ", " << i2 << ") on const graph with return value "
	   << flush;
  ecit = constGraph.internalFindEdge (i1, i2);
  if (constGraph.edge_end () != ecit)
    {
      gOut (0) << *ecit;
    }
  ...
  // Testing internalFindEdgeWeight
  i1 = graph.size ();
  i2 = 8;
  gOut (0) << "internalFindEdgeWeight (" << i1 << ", " << i2 << ") with return value "
	   << flush;
  ewit = graph.internalFindEdgeWeight (i1, i2);
  if (graph.eweight_end () > ewit)
    {
      gOut (0) << *ewit;
    }
  ...
  i1 = constGraph.size ();
  i2 = 8;
  gOut (0) << "internalFindEdgeWeight (" << i1 << ", " << i2 << ") on const graph with return value "
	   << flush;
  ewcit = constGraph.internalFindEdgeWeight (i1, i2);
  if (constGraph.eweight_end () > ewcit)
    {
      gOut (0) << *ewcit;
    }
  ...
  // Testing set internalFindEdgeWeight
  i1 = graph.size ();
  i2 = 8;
  v4 = 5.6;
  gOut (0) << "set internalFindEdgeWeight (" << i1 << ", " << i2 << ", " << v4 << ") "
	   << flush;
  ewit = graph.internalFindEdgeWeight (i1, i2);
  if (graph.eweight_end () != ewit)
    {
